...
export const Route = createFileRoute(route.settings.engine_settings as any)({
...
const SECTION_KEYS: Record<string, string[]> = {
  ...
  attention: ['flash_attn', 'cont_batching', 'ctx_shift', 'cache_type_k', 'cache_type_v', 'defrag_thold'],
...
function EngineSettingsContent() {
  ...
  const renderSection = (sectionKey: string, keys: string[]) => {
    ...
    const sectionSettings = keys
      ...
      .filter(Boolean) as { setting: (typeof provider.settings)[0]; index: number }[]
  ...
  return (
    <div className="flex flex-col h-svh w-full">
      <HeaderPage>
        <div className={cn('flex items-center gap-2 w-full', !IS_MACOS && 'pr-30')}>
          ...
          <div className="flex flex-col justify-between gap-4 gap-y-3 w-full">
            <div className="flex items-center justify-between">
              <h1 className="font-medium text-base">
                {t('settings:engineSettings.llamacpp')}
              </h1>
            </div>
