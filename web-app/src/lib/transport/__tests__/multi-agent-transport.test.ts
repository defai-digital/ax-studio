...
vi.mock('@/lib/multi-agent/delegation-tools', () => ({
  buildDelegationTools: vi.fn().mockReturnValue({}),
}))

vi.mock('@/lib/multi-agent/parallel-orchestration', () => ({
  buildParallelOrchestration: vi.fn().mockReturnValue({ tools: {}, system: 'parallel system' }),
}))

vi.mock('@/lib/multi-agent/orchestrator-prompt', () => ({
...
vi.mock('@/lib/multi-agent/sanitize', () => ({
