...
import {
  ...
  estimateTeamRunCost,
} from '@/features/multi-agent/lib'
import type { CostEstimate } from '@/features/multi-agent/lib/cost-estimation'
import { buildDelegationTools } from '@/features/multi-agent/lib/delegation-tools'
import { buildParallelOrchestration } from '@/features/multi-agent/lib/parallel-orchestration'
import {
  buildOrchestratorPrompt,
  resolveVariables,
} from '@/features/multi-agent/lib/orchestrator-prompt'
import { sanitize } from '@/features/multi-agent/lib/sanitize'
...
export async function executeMultiAgentStream(
  ...
  try {
    ...
    const providerConfig = useModelProvider
      .getState()
      .getProviderByName(providerId)
    const activeModelId =
      modelOverrideId ?? useModelProvider.getState().selectedModel?.id
    ...
    const variableValues = (rawMeta.agent_team_variables ?? {}) as Record<
      string,
      string
    >
        ...
        switch (mode) {
          ...
          case 'sequential': {
            if (delegationsMade < agents.length) {
              stepToolChoice = {
                type: 'tool',
                toolName: `delegate_to_${sanitize(agents[delegationsMade].name)}`,
              }
            }
            break
          }
              ...
              if (steps && steps.length > 0) {
                ...
                const resultText =
                  typeof lastToolResult?.content === 'string'
                    ...
                    : ''
                if (
                  /\bapproved?\b/i.test(resultText) ||
                  /\bpass(ed)?\b/i.test(resultText)
                )
                  break
              }
    ...
    const cleanedMessages = stripUnavailableToolParts(
      options.messages,
      new Set(Object.keys(config.tools))
    )
      ...
      execute: async ({ writer }) => {
        ...
        try {
                  ...
                  onTokenUsage(
                    {
                      ...
                      totalTokens:
                        usage.consumed || (totalUsage?.totalTokens ?? 0),
                    },
                    options.messageId ?? ''
                  )
                }
          ...
          if (/bad request|400|unsupported.*tool.?choice/i.test(errMsg)) {
            ...
            hint =
              ' This model requires strict user/assistant role alternation which the multi-agent orchestrator cannot guarantee. Try a model with flexible message ordering (e.g. GPT-4o, Claude, Gemini).'
          } else if (/type validation failed/i.test(errMsg)) {
            hint =
              ' The model returned an unexpected response format. This often happens with local models. Try a cloud model with standard OpenAI-compatible responses.'
          }

          // Extract a cleaner message from verbose Type validation errors
          let displayMsg = errMsg
          if (
            /^Type validation failed/i.test(displayMsg) &&
            displayMsg.length > 200
          ) {
            const innerMatch = displayMsg.match(
              /llama-server chat error \d+ [^:]+: (.*?)(?:","type")/s
            )
    ...
    return createUIMessageStream({
      execute: async ({ writer }) => {
        writeTextMessage(
          writer,
          'agent-error-notice',
          `> **Agent Team Error:** ${errMsg}. Falling back to single-agent mode.\n\n`
        )
        writer.merge(fallbackStream)
      },
    })
  }
}
