#!/bin/bash
# Ubuntu download script for Ax-Studio app
...
DOWNLOAD_PATH="/tmp/ax-studio-installer.deb"

# Download the package
if ! wget "$JAN_APP_URL" -O "$DOWNLOAD_PATH"; then
    echo "Failed to download Ax-Studio app"
    exit 1
fi

if [ -f "$DOWNLOAD_PATH" ]; then
    FILE_SIZE=$(stat -c%s "$DOWNLOAD_PATH")
    echo "Downloaded Ax-Studio app successfully. Size: $FILE_SIZE bytes"
