#!/bin/bash
# Ubuntu download script for Ax-Fabric app
...
DOWNLOAD_PATH="/tmp/ax-fabric-installer.deb"

# Download the package
if ! wget "$JAN_APP_URL" -O "$DOWNLOAD_PATH"; then
    echo "Failed to download Ax-Fabric app"
    exit 1
fi

if [ -f "$DOWNLOAD_PATH" ]; then
    FILE_SIZE=$(stat -c%s "$DOWNLOAD_PATH")
    echo "Downloaded Ax-Fabric app successfully. Size: $FILE_SIZE bytes"
