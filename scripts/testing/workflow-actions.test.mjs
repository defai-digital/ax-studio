...
describe('GitHub Actions dependency boundaries', () => {
  ...
  it('requires and verifies the Homebrew stable release path', () => {
    ...
    expect(releaseWorkflow).toContain('defai-digital/homebrew-tap.git')
    expect(releaseWorkflow).toContain('brew install --cask defai-digital/tap/ax-studio')
    expect(releaseWorkflow).toContain('git pull --rebase origin main')
    ...
    expect(caskWriter).toContain('FileUtils.rm_r(legacy_app) if File.exist?(legacy_app)')
